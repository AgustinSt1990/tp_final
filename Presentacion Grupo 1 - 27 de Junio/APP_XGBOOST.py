...
def load_dataset(file_name):
    logger.info(
        f"LOADING FUNCTION: load_dataset(): se carga el dataset llamado {file_name}"
    )
...
def series_to_supervised(data, n_in=1, n_out=1, dropnan=True):
    ...
    logger.info(
        "SET DATA FUNCTION: series_to_supervised(): se transforma una serie de datos en una matriz de datos supervisados"
    )
...
def walk_forward_validation(data, n_test, visualization=False):
    ...
    logger.info(
        'MODEL FUNCTION: walk_forward_validation(): funcion principal - queda almacenada en "app_xgboost_forecast.pickle"'
    )
...
def train_test_split_st(data, n_test):
    ...
    logger.info(
        "SET DATA FUNCTION: train_test_split_st(): se divide un dataset univariado en train y test"
    )
...
def xgboost_forecast(history, testX):
    ...
    logger.info(
        'MODEL FUNCTION: xgboost_forecast(): se entrena el modelo "xgboost N {}" y se realiza la prediccion de un paso'.format(
            x_1()
        )
    )
...
def guardar_funcion():
    ...
    logger.info(
        "SAVE FUNCTION: guardar_funcion(): se guarda la funcion en un archivo pickle"
    )
